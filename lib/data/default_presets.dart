...
bool _isLegacyMicuPropofolLabel(DrugPreset preset) {
  final nameMatches = preset.name.trim().toLowerCase() == 'propofol 200mg';
  final amountIs400Mg = (preset.drugUnit.trim().toLowerCase() == 'mcg' &&
          preset.drugAmount == 400000) ||
      (preset.drugUnit.trim().toLowerCase() == 'mg' &&
          preset.drugAmount == 400);
  return nameMatches && amountIs400Mg && preset.volumeMl == 40;
}

List<DrugPreset> replaceLegacyMicuPropofolLabel(List<DrugPreset> presets) {
  return presets.map((preset) {
    if (!_isLegacyMicuPropofolLabel(preset)) return preset.copy();
    return DrugPreset(
      id: preset.id,
      name: 'propofol 400mg',
      doseUnit: preset.doseUnit,
...
DrugPreset buildDrugPreset({
  ...
  return DrugPreset(
    ...
    rateIncrementMlPerHr: rateIncrementMlPerHr ??
        (name.trim().toLowerCase().startsWith('heparin') ? 1.0 : 0.1),
  );
}
...
bool shouldReplaceLegacyEicu2Presets(List<DrugPreset> presets) {
  ...
  const currentMarkers = {'norepinephrine (central)', 'vasopressin', 'heparin'};
